...
parser.add_argument('--lambda_1', default=0.8, type=float, help='weight of Enhanced Cross Triplet Loss')
...
if dataset == 'sysu':
    data_path = '/root/autodl-tmp/cross_id/RFM-main/sysu-mm01/'
    log_path = args.log_path + 'sysu_log/'
    test_mode = [1, 2]  # thermal to visible
elif dataset == 'regdb':
    data_path = '/root/autodl-tmp/cross_id/RFM-main/RegDB/'
...
criterion_ect = ECTLoss(margin=0.2)

criterion_id.to(device)
criterion_tri.to(device)
criterion_cc.to(device)
criterion_ect.to(device)
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2, cam1, cam2) in enumerate(trainloader):
        ...
        cam_ids = torch.cat((cam1, cam2), 0).cuda()
        feat_zq,feat, output, feat_globe, out_globe = net(input1, input2,0,cam_ids)
        ...
        ft1, ft2, ft3, ft4, ft5, ft6 = torch.chunk(feat_zq, 6, 0)

        loss_ect = (criterion_ect(torch.cat((ft1, ft2), 0), labs) + criterion_ect(torch.cat((ft1, ft3), 0), labs) + criterion_ect(torch.cat((ft1, ft4), 0), labs)
                    + criterion_ect(torch.cat((ft1, ft5), 0), labs) + criterion_ect(torch.cat((ft1, ft6), 0), labs)) * args.lambda_1

        unad_loss = net.module.unad_loss if hasattr(net.module, 'unad_loss') else 0
        pseu_loss = net.module.pseu_loss if hasattr(net.module, 'pseu_loss') else 0
        ...
        loss = loss_id + loss_tri + loss_cc + loss_globe + cc_unad*unad_loss + cc_pseu*pseu_loss + loss_ect
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        ect_loss.update(loss_ect.item(), 2 * input1.size(0))
